...
class ChatStore {
	...
	private async streamChatCompletion(
		...
		const Service = useResponsesApi ? OpenResponsesService : ChatService;
	...
	async sendMessage(content: string, extras?: DatabaseMessageExtra[]): Promise<boolean> {
		if (!content.trim() && (!extras || extras.length === 0)) return false;
		const activeConv = conversationsStore.activeConversation;
		if (activeConv && this.isChatLoading(activeConv.id)) return false;
		...
		try {
			...
			conversationsStore.addMessageToActive(assistantMessage);
				...
				if (this.isAbortError(error)) {
					if (convId) this.setChatLoading(convId, false);
					return true;
				}
				...
				if (
					...
					error.name === 'ConversationStorageError'
				) {
					return false;
				}
