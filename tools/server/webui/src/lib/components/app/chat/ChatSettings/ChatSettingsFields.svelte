<script lang="ts">
	...
	function getDefaultPlaceholder(key: string) {
		if (sp[key] != null) {
			return t('chat.settings.default_value', {
				value: String(normalizeFloatingPoint(sp[key]))
			});
		}
	...
	function getResetValue(key: string) {
		return sp[key] != null ? '' : String(SETTING_CONFIG_DEFAULT[key] ?? '');
	}
				...
				{#if isCustomRealTime}
					<button
						...
						onclick={() => {
							...
							onConfigChange(field.key, getResetValue(field.key));
						}}
